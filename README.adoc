...
In commercial uses, especially in financial services, there can be a high degree of trust between the participants - say, a banking consortium -
and regulatory requirements that users _not_ be anonymous - "Know Your Customer" (KYC) and "Anti Money Laudering" (AML).
...
Fundamentally, transactions transfer ownership of some piece of property from one person to another.
That could be a car title, a piece of artwork, or a house.
...
So there's a lot of potential here, but there are a lot of things to worry about.
The mathematics of Blockchain may be flawless and elegant, but people aren't.

The biggest concern for any consumer-facing Blockchain is key management.
Even within the Bitcoin community, which is mostly pretty tech-savvy, there are numerous horror stories of private keys lost through mistakes, drive failures, etc., rendering thousands or millions of dollars worth of BTC unspendable.

Private keys can also be stolen by anyone able to hack into your machine and copy out a file.
That's a pretty low bar in the security world.
The key can be password protected, but you know how bad most people are at setting good passwords.

Even aside from theft, there are plain mistakes.
Typo an amount, or send it to the wrong address.
Once it's gone, it's gone.
There's no way to undo a transaction.
You can ask the recipient to transfer the value back to you.
If you know who they are.
Without some outside governing authority, there's no way to make them.

Blockchain also doesn't allow for simple contested charges.
The Blockchain transaction is usually only half of the transaction: goods and/or services are also exchanged in the real world.
Unless the transaction has an arbitrator, as described above, there's no recourse for the buyer if they aren't happy with what they got.

== System Problems

Again, the mathematics of Blockchain may be flawless and elegant, but the software that implements it may not be.

Depending on the use case, the storage required for the Blockchain can become an issue.
It's an ever-growing database of every transaction ever.

Private Blockchains with different consensus algorithms can be faster, but Bitcoin's proof-of-work limits it to about 7 transactions per second.
Compare this to 115 for Paypal and 2000-4000 for Visa.
...
* not consumer-facing because of key management
* "can't rewrite history" is appealing for financial services
* but need some way to reverse transactions - good to leave them in record
* block validation good for widely distributed/high-latency network
